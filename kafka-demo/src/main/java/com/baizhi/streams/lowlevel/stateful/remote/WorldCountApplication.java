...
public class WorldCountApplication {
    public static void main(String[] args) {
         ...
         * 数据删除策略
         * compact(紧凑)  新值覆盖旧值
         * delete(默认删除策略)   数据保留周期默认是七天，写数据的时候会不断往topic分区中追加数据
         */
        changelog.put("cleanup.policy","compact");
        ...
        StoreBuilder<KeyValueStore<String, Long>> countStoreSupplier = Stores.keyValueStoreBuilder(
                ...
                .withLoggingEnabled(changelog); // 开启remote state store(远程状态备份)   disable backing up the store to a changelog topic
